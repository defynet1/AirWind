...
После любых изменений в `server.js` или `index.html` обязательно запускай:

```bash
wsl -d Ubuntu bash -c "cd /mnt/c/Users/MakRoyl/Documents/AirWind && bash healthcheck.sh"
```

Скрипт проверяет HTTP (код 200) и WebSocket-соединение.
...
- **SQLite** (через `sql.js`) — база в памяти, загружается из `airwind.db` и периодически сохраняется в него
...
**Сохранение БД**: запись дебаунсится на 400 мс через `flush()`, которая экспортирует in-memory БД sql.js в файл `airwind.db`.
